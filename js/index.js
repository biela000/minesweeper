...
function settingsSubmitHandler(event) {
  ...
  const usernameValue = minesweeperSettingsInputs.username.value;
  ...
  const areInputsValid = {
    username: usernameValue.trim().length > 0,
    width: widthValue > 0,
  ...
  if (!areInputsValid.username) {
    minesweeperSettingsInputs.username.classList.add("invalid");
  } else {
    minesweeperSettingsInputs.username.classList.remove("invalid");
  }
  ...
  if (minesweeperContainer !== null) {
    flagCountElement.removeChild(document.querySelector(".flag-count-text"));
...
class MineSweeper {
  ...
  generateBombs(vIndex, hIndex) {
    ...
    for (let i = 0; i < this.mineCount && workBoxes.length > 0; i++) {
      ...
      if (randomIndexes[0] > 0) {
        this.boxes[randomIndexes[0] - 1][randomIndexes[1]].addNearbyBomb(
          randomIndexes[0] - 1,
          randomIndexes[1]
        );
      }
      ...
      if (randomIndexes[1] > 0) {
        this.boxes[randomIndexes[0]][randomIndexes[1] - 1].addNearbyBomb(
          randomIndexes[0],
          randomIndexes[1] - 1
        );
        if (randomIndexes[0] > 0) {
          this.boxes[randomIndexes[0] - 1][randomIndexes[1] - 1].addNearbyBomb(
            randomIndexes[0] - 1,
            randomIndexes[1] - 1
          );
        }
      ...
      for (let o = 0; o < this.width; o++) {
        this.boxElements[i][o].addEventListener("click", () => {
          if (
            ...
            this.boxes[i][o].flagsAround === this.boxes[i][o].bombsAround
          ) {
        ...
        this.boxElements[i][o].addEventListener("contextmenu", (event) => {
          event.preventDefault();
          this.CFS(i, o, "FLAG");
        });
      }
  ...
  openBox(vIndex, hIndex) {
    if (!this.boxes[vIndex][hIndex].isBomb) {
      ...
      if (this.uncoveredCount === this.height * this.width - this.mineCount) {
        this.winGame();
      }
    } else {
      this.boxes[vIndex][hIndex].isUncovered = true;
  ...
  CFS(vIndex, hIndex, specialType = "") {
    if (
      ...
      if (specialType === "FLAG" && !this.boxes[vIndex][hIndex].isUncovered) {
        const flagCountText = document.querySelector(".flag-count-text");
    ...
    CookieHandler.post({
      ...
      score: Math.ceil(
        (this.mineCount /
          ...
          (timerSeconds > 0 ? timerSeconds : 1 / (this.width * this.height))) *
          100000
      ),
    });
    clearInterval(timerInterval);
